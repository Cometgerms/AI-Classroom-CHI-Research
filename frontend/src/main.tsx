...
function App(){
  ...
  useEffect(()=>{refresh();fetch(API+'/api/config').then(r=>r.json()).then(setConfig);const id=setInterval(refresh,750);return()=>clearInterval(id)},[]);
  if(!state) return <div className="loading">Connecting to classroom brain…</div>;

  const condition:Condition=state.condition;
  const inject=async(name:string)=>{setBusy(true);try{await post('/api/scenario/'+name);await refresh()}finally{setBusy(false)}};
  ...
  return <div className="app">
    ...
    <main className={research?'grid withResearch':'grid'}>
      ...
      <section className="panel agentPanel">
        ...
        {condition==='assistive' && !recommendation && <div className="agentMessage">Inject a scenario to request an AI recommendation.</div>}
      ...
      {research && <aside className="panel research">
